...
function Movie() {
    ...
    const url = `https://api.themoviedb.org/3/movie/${id}?api_key=${process.env.REACT_APP_API_KEY}&language=en-US&append_to_response=videos`
    ...
    const handleTrailer = () =>{
        const getTrailer = details.videos.results.find(vid => vid.name === 'Official Trailer'|| vid.name ==='official trailer')
        if(!getTrailer){
            setIsError(true)
        }else{
